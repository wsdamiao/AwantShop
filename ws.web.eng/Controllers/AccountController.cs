...
namespace ws.web.eng.Controllers
{
    ...
    public class AccountController : Controller
    {
            ...
            if (usu.NomeUsuario != null)
            {
                if (usu.Ativo)
                {
                    ...
                    if (usu.Categoria.ID == 5)
                    {
                        ...
                        usu.NomeCompleto = usu.Cliente.Nome;
                        return RedirectToAction("Index", "Dashboard");
                    }
                    else
                    {
                        model.ClienteName = usu.NomeCompleto;
                        return RedirectToAction("Index", "Manager");
                    }
                }
                else
                {
                    TempData["UsuarioId"] = usu.ID;
        ...
        {
            LoginValidatorModel model = new LoginValidatorModel();
            UsuarioClienteDll usuDll = new UsuarioClienteDll();
            ...
            model.Usuario = usu.NomeUsuario;
            model.NomeUsuario = usu.NomeCompleto;
            model.UsuarioId = usu.ID;
        ...
        [HttpPost]
        [AllowAnonymous]
        public ActionResult VerifyUser(LoginValidatorModel model)
        {
            if (ModelState.IsValid)
            {
                UsuarioObj usu = new UsuarioObj();
                ...
                if (model.NovaSenha == model.RepetirNovaSenha)
                {

                    usu = usuDll.BuscarUsuario(model.UsuarioId);
                    usu.Ativo = true;
                ...
                {
                    TempData["Message"] = "As senhas não são identicas";
                    ViewBag.Message = TempData["Message"];
                    TempData["UsuarioId"] = model.UsuarioId;
                }
            }
